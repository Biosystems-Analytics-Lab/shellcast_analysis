...
def create_log_files(state):
    log_dir = os.path.join(ct.LOGS_DIR, state.lower())
...
def send_email(state, message):
    try:
        em = EmailMessage()
        em['From'] = ct.EMAIL_SENDER
        em['To'] = ct.EMAIL_RECEIVER
        em['Subject'] = f'{state} {ct.EMAIL_SUBJECT}'
        em.set_content(message)
        context = ssl.create_default_context()
        with smtplib.SMTP_SSL('smtp.gmail.com', 465, context=context) as smtp:
            smtp.login(ct.EMAIL_SENDER, ct.EMAIL_PASSWORD)
            smtp.sendmail(ct.EMAIL_SENDER, ct.EMAIL_RECEIVER, em.as_string())
    except Exception as e:
        error_log(logger, e)
        exit(1)
